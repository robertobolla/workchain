...
export const ASK_AI_CHAT = async (prompt) => {
  ...
  while (attempts < maxAttempts) {
    try {
      ...
      if (error.response?.status === 429) {
        attempts += 1;
        console.warn(
          `Attempt ${attempts} failed due to rate limit. Retrying after delay...`
        );
        await delay(5000);
      } else if (error.response?.status === 403) {
        console.error("Permission error accessing model.");
        throw new Error("Access to the model is restricted.");
      } else {
  ...
  throw new Error(
    "Límite de solicitudes alcanzado. Intente de nuevo más tarde."
  );
};
